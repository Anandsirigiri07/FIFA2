    ...
    <header className="border-b border-slate-800 bg-slate-950/80 backdrop-blur-md sticky top-0 z-40 px-4 py-3.5">
      <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="flex items-center space-x-3.5">
          <div className="bg-fifa-gold/15 p-2 rounded-lg border border-fifa-gold/30">
            <Trophy className="w-6 h-6 text-fifa-gold" aria-hidden="true" />
          </div>
          <div>
            <h1 className="text-xl font-extrabold font-outfit text-white tracking-tight flex items-center gap-1.5">
        ...
        <div className="flex items-center flex-wrap justify-center gap-3">
          <div className="flex items-center space-x-2 bg-slate-800 border border-slate-700 rounded-lg px-2.5 py-1.5 focus-within:border-fifa-gold transition-colors">
            <select
              value={activeVenueId}
              onChange={(e): void => onVenueChange(e.target.value)}
          ...
          {onNavigateToAccessibility && (
            <button
              ...
              className="flex items-center justify-center p-2 text-slate-400 hover:text-fifa-gold bg-slate-800 hover:bg-slate-700/80 rounded-lg transition-colors border border-slate-700"
